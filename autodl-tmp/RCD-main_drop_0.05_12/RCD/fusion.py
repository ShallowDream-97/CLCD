...
class Fusion(nn.Module):
    def __init__(self, args, local_map):
        ...
        self.u_from_e_per = remove_random_edges(local_map['u_from_e'],args.dropout).to(self.device)
        ...
        self.u_from_e = LightGCN(self.u_from_e, args.knowledge_n, args.knowledge_n)  # src: e
        self.e_from_u = LightGCN(self.e_from_u, args.knowledge_n, args.knowledge_n)  # src: u
        self.u_from_e_per = GraphLayer(self.u_from_e_per, args.knowledge_n, args.knowledge_n)  # src: k
    ...
    def forward(self, kn_emb, exer_emb, all_stu_emb):
        ...
        all_stu_emb_per = all_stu_emb + u_from_e_graph_per[self.exer_n:]

        return kn_emb, exer_emb, all_stu_emb,all_stu_emb_per
    
class Fusion_Ori(nn.Module):
    def __init__(self, args, local_map):
        self.device = torch.device(('cuda:%d' % (args.gpu)) if torch.cuda.is_available() else 'cpu')
        self.knowledge_dim = args.knowledge_n
        self.exer_n = args.exer_n
        ...
        super(Fusion_Ori, self).__init__()
