...
def divide_data():
    ...
    :return:
    '''
    with open('../data/ASSIST/0.05_1024_log_data.json', encoding='utf8') as i_f:
        stus = json.load(i_f)
    # 1. delete students who have fewer than min_log response logs
    stu_i = 0
    l_log = 0
    ...
    with open('../data/ASSIST/0.05_1024_train_data.json', 'w', encoding='utf8') as output_file:
        json.dump(train_set, output_file, indent=4, ensure_ascii=False)
    with open('../data/ASSIST/0.05_1024_test_data.json', 'w', encoding='utf8') as output_file:
