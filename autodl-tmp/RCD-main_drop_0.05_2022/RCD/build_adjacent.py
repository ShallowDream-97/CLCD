...
class LightGCN(nn.Module):
    def __init__(self, num_users, num_items, input_dim, hidden_dims, laplacian):
        ...
        self.layers.append(nn.Linear(input_dim, hidden_dims[0]))  # 初始化层

        for i in range(len(hidden_dims) - 1):
            self.layers.append(nn.Linear(hidden_dims[i], hidden_dims[i + 1]))

        self.gcn_layers = nn.ModuleList([LightGCNLayer() for _ in range(len(hidden_dims))])

        self.laplacian = laplacian


    def forward(self, x):
        for linear, gcn in zip(self.layers, self.gcn_layers):
            x = linear(x)
            x = torch.relu(x)
...
hidden_dims = [64, 32]  # example hidden layer dimensions
model = LightGCN(input_dim, hidden_dims, normalized_laplacian_matrix)
